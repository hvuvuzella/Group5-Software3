...
# Define a function that send request to endpoint to add new client by sending an HTTP POST request with client
# information
...
def run():
    stylist_or_customer = input("Welcome to THE CUT, where you can get everything you need for your hair.\nPrint "
    ...
    if stylist_or_customer == "s":
        ...
        if len(stylist_schedule['data']) == 0:
            print("You have no appointments booked for this day")
        else:
            stylist_bookings = []
            ...
            print(f'You have the following bookings for this day:\n{"".join(stylist_bookings)}')
    # If the user logs in as a customer, they are prompted to register by providing their information
    if stylist_or_customer == "c":
        registered_user = input("Are you a registered customer with us?(y/n) ")
        if registered_user == "n":
            client_name = input(
                "To continue with bookings, you will need to register. Please Enter your first name: ")
            ...
            print(
                f"Congratulations! You are registered with a customer ID: {client_id['customer_id']}. Please make note of this ID to make, update and cancel your bookings.")
            ...
            for booking in bookings["data"]:
                all_bookings.append(
                    f"{booking['name']} {booking['last_name']} (customer id {booking['app_id']}) - booking for {booking['treatment']} with booking id {booking['app_id']} for {booking['date']} at {booking['time']}")
            print(f'Your bookings are:\n{". ".join(all_bookings)}')

        customer_choice = input("Type a if you want to make a new booking with us,"
                                "OR d if you want to cancel an existing booking (using your booking ID).")
